...
define('appdeeplink/appDeeplink', ['common/config','tools/deeplink'], function(CONFIG,Deeplink){
    return {
        ...
        init: function(options) {
            ...
            if(this.itemcode===''){
                window.location.replace(this.hostName);
            }
            this.pid = this.getUrlParam('pid') || '';
            //获取类目名称
            this.cateId = this.getUrlParam('seoname');
            if(this.cateId===''){
                this.cateId='deeplink';
            }
            //配置对象初始化
        ...
        setOptions: function(options) {
            this.options = {
			    ...
			    android:{
		            ...
		            appName: "DHgate",
                    storeUrl:"http://app.appsflyer.com/com.dhgate.buyer-dhgate?pid="+this.pid+"&af_sub1=itemcode&af_sub2="+this.itemcode+"&af_r="+this.hostName+"/product/"+this.cateId+"/"+this.itemcode+".html"
		        },
			    //设置android的直接下载地址
			    androidDownloadUrl:"http://app.appsflyer.com/com.dhgate.buyer-dhgate?pid="+this.pid+"&af_sub1=itemcode&af_sub2="+this.itemcode+"&af_r="+this.hostName+"/product/"+this.cateId+"/"+this.itemcode+".html",
                //设置ios的直接下载地址
                iosDownloadUrl:this.hostName+"/product/"+this.cateId+"/"+this.itemcode+".html",
			    //设置唤起app的schemeUrl
