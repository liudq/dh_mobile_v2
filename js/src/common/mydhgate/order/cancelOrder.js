/**
 * module src: common/mydhgate/order/cancelOrder.js
 * 取消订单模块
**/
define('mydhgate/order/cancelOrder',['common/config','lib/backbone','checkoutflow/popupTip','checkoutflow/dataErrorLog'],function(CONFIG,Backbone,tip,dataErrorLog){
    //model-取消订单
    ...
    //view-取消订单
    var cancelOrderView = Backbone.View.extend({
        //根节点
        el: 'body',
        //backbone提供的事件集合
        events: {
            'click .pay_cancel': 'cancelOrderByRfid'
        },
        //初始化入口
        ...
        success: function(model,response,options) {
            ...
            if (model.get('code') === 200) {
                ...
                //提示取消订单后刷新页面
                tip.events.trigger('popupTip:autoTip',{message:'Your order has been canceled.',callback:function(){
                ...
                tip.events.trigger('popupTip:dataErrorTip', {action:'refresh',message:response.message||'Sorry, order cannot be cancelled.'});
                //刷新当前页
                self.toCancelArea();
                //捕获异常
                try {
        ...
        //取消订单
        cancelOrderByRfid: function(evt) {
            //确定是否取消订单
