/**
 * module src: cart/removeCart.js
 * 移除购物车列表模块
**/
define('app/removeCart', ['common/config','lib/backbone','app/getCartList','checkoutflow/popupTip','checkoutflow/dataErrorLog'], function(CONFIG,Backbone,getCartList,tip,dataErrorLog){
    //model-移除购物车列表模块
    var removeCartModel = Backbone.Model.extend({
        //移除购物车列表模块
        defaults: function() {
            return {
                //状态码
    ...
    //model-移除购物车列表模块
    var removeCartView = Backbone.View.extend({
        //根节点
        el:'.j-wrap-order',
        //backbone提供的事件集合
        events: {
            'click .j-odDelete': 'removeCart'
        },
        //初始化入口
        ...
        deleteItem: function(evt){
            ...
            if(len){
                $prevParent.find(".j-orderTotal").text($prevParent.find(cJOrderDetails).length);
            }else{
                $prevParent.remove();
            }
        ...
        getPrice:function(){
            ...
            i = cartList.getCheckNum();
            if(i==0){
                $cJcheckOut.closest($cJCheckOutWap).addClass(cNoClick);
            }else{
                $cJcheckOut.closest($cJCheckOutWap).removeClass(cNoClick);
            }
        }
    });
    
    return removeCartView;
});
