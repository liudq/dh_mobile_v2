/**
 * module src: cart/cartSaveForLater.js
 * 购物车列表点击saveForLater按钮移到saveForLater列表模块
**/
define('app/cartSaveForLater', ['common/config','lib/backbone','app/getSaveForLater','app/getCartList','checkoutflow/popupTip','checkoutflow/dataErrorLog'], function(CONFIG,Backbone,getSaveForLater,getCartList,tip,dataErrorLog){
    //model-移除购物车列表模块
    var cartSaveForLaterModel = Backbone.Model.extend({
        //移除购物车列表模块
        defaults: function() {
            return {
                //状态码
        ...
        **/
        ...
        setOptions: function(options) {
            this.options = {
                //$.ajax()配置对象
                ajaxOptions: {
                    url: CONFIG.wwwURL + '/mobileApiWeb/cart-Cart-save4Later.do',
                    //url: 'removeCart.json',
                    data: {
                        version: 3.3,
                        client: 'wap',
    ...
    //model-移除购物车列表模块
    var cartSaveForLaterView = Backbone.View.extend({
        //根节点
        el:'.j-wrap-order',
        //backbone提供的事件集合
        events: {
            'click .j-saveForLater': 'cartSaveForLater'
        },
        //初始化入口
        initialize: function(options) {
            ...
            this.cJOdDelete = this.options.cJOdDelete;
        ...
        setOptions: function(options) {
            this.options = {
                ...
                cChecked:'checked',
                //数据模型
                model: new cartSaveForLaterModel(),
               //购物车列表实例对象
                cartList: $({}),
        ...
        //购物车列表移除到saveForLater
        cartSaveForLater:function(e){
            //显示loadding弹层
